...
pub const JXA_SHEBANG: &str = "#!/usr/bin/osascript -l JavaScript";
...
pub fn detect_script_file_from_bytes(path: &Path, prefix_bytes: Option<&[u8]>) -> ScriptFileInfo {
    let is_jxa = has_scripta_jxa_shebang(prefix_bytes);
...
fn has_scripta_jxa_shebang(prefix_bytes: Option<&[u8]>) -> bool {
    prefix_bytes.is_some_and(|bytes| bytes.starts_with(JXA_SHEBANG.as_bytes()))
}
...
fn supports_inline_scriptmeta(path: &Path) -> bool {
    extension_matches(
        path,
        &["js", "jsx", "jsxinc", "applescript", "idjs", "jxa", "psjs"],
    )
}
